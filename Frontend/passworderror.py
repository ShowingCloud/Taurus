...
class PasswordError (QtGui.QDialog):
...
#		self.setWindowTitle (self.tr ("Forget Password"))
	...
	def keyPressEvent (self, event):
		super (PasswordError, self).keyPressEvent (event)
	...
	def mouseMoveEvent (self, event):
		super (PasswordError, self).mouseMoveEvent (event)
		if self.leftclicked == True:
			self.move (event.globalPos() - self.startdragging)

	def mousePressEvent (self, event):
		super (PasswordError, self).mousePressEvent (event)
		if event.button() == QtCore.Qt.LeftButton:
			self.leftclicked = True
			self.startdragging = event.pos()

	def mouseReleaseEvent (self, event):
		super (PasswordError, self).mouseReleaseEvent (event)
