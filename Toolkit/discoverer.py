        ...
        if message.type == gst.MESSAGE_EOS:
            ...
            if 'bitrate' in taglist.keys():
                if 'video-codec' in taglist.keys():
                    self.videobitrate = taglist['bitrate']
                elif 'video' in message.src.get_name():
                    self.videobitrate = taglist['bitrate']
                if 'audio-codec' in taglist.keys():
                    self.audiobitrate = taglist['bitrate']
                elif 'audio' in message.src.get_name():
                    self.audiobitrate = taglist['bitrate']
            if 'video-codec' in taglist.keys():
                self.videocodec = taglist['video-codec']
            if 'audio-codec' in taglist.keys():
                self.audiocodec = taglist['audio-codec']
            if 'segmentlength' in taglist.keys():
                self.segmentlength = int (taglist['segmentlength'])
